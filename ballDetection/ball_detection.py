...
def write_track(frames, ball_track, path_output_video, fps, trace=1):
        ...
        for i in range(trace):
            if (num-i > 0):
                if ball_track[num-i][0]:
                    x = int(ball_track[num-i][0]) #* 0.666
                    y = int(ball_track[num-i][1]) #* 0.66
                    # frame = cv2.circle(frame, (x,y), radius=0, color=(0, 0, 255), thickness=10-i)
                    frame = cv2.circle(frame, (x,y), radius=0, color=(0, 0, 255), thickness=12)
                else:
                    break
        out.write(frame) 
    out.release()    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_path', type=str, default = './tennis_3.mp4' ,help='path to input video')
    parser.add_argument('--video_out_path', type=str, default = './tennis_3_output.mp4',help='path to output video')
    parser.add_argument('--extrapolation', action='store_true', default = True, help='whether to use ball track extrapolation')
